...
public class BitmapUtilities {
	...
	public Bitmap loadAssetBitmap(Context context, String fileName) {
		byte[] bufferData = createByteArrayFromAsset(context, fileName);
		if(bufferData != null) {
			return BitmapFactory.decodeByteArray(bufferData, 0, bufferData.length);
		}
		else {
			return null;
		}
	...
	public Bitmap loadResourceBitmap(Resources res, int resourceId) {
	...
	public Bitmap loadResizedResourceBitmap(Resources res, int resourceId, int reqWidth, int reqHeight) {
	...
	/*Target resolution should be equal or less the resolution from the image to open.*/
	public Bitmap loadResizedAssetBitmap(Context context, String fileName, int reqWidth, int reqHeight) {
